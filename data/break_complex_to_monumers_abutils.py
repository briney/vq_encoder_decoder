...
def sequence_similarity(seq1, seq2):
    ...
    aln = abutils.tl.global_alignment(seq1, seq2)
    n_matches = sum([m == "|" for m in aln.alignment_midline])
    similarity = n_matches / min(len(seq1), len(seq2))
    return similarity
    ...
    for chain_id, sequence in chain_sequences.items():
        ...
        for existing_sequence in sequence_to_chain.keys():
            if sequence_similarity(sequence, existing_sequence) > similarity_threshold:
                is_similar = True
                existing_chain_id, existing_ca_count = sequence_to_chain[
                    existing_sequence
                ]
...
def preprocess_file(
    ...
    stats = Counter()
    parser = (
        MMCIFParser(QUIET=True, auth_chains=False) if use_cif else PDBParser(QUIET=True)
    )
    ...
    for chain_id in best_chains.keys():
        ...
        if with_file_index:
            outputfile = os.path.join(
                save_path, f"{file_index}_{basename}_{chain_suffix}.pdb"
            )
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Split complexes into monomeric PDB files."
    )
    parser.add_argument(
        "--data", default="./test_data", help="Path to structure files."
    )
    parser.add_argument(
        "--max_len", default=2048, type=int, help="Max sequence length to consider."
    )
    parser.add_argument(
        "--save_path", default="./save_test/", help="Path to output PDB files."
    )
    parser.add_argument(
        "--max_workers",
        default=16,
    ...
    parser.add_argument(
        "--use_cif", action="store_true", help="Use CIF/mmCIF input instead of PDB."
    )
    parser.add_argument(
        "--with_file_index",
        action="store_true",
    ...
    with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for future in tqdm(
            ...
            try:
                ...
                print(
                    f"An error occurred while processing {file_path}: {exc} {type(exc)}"
                )
